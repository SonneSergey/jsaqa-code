...
When('I click the {string} button', async function (buttonText) {
    ...
    const btnText = await this.page.$eval('.acceptin-button', el => el.textContent.trim());
    if (btnText !== buttonText) {
        throw new Error(`Expected button text "${buttonText}", but got "${btnText}"`);
    }
    await this.page.click('.acceptin-button');
});

// Подтверждения
Then('I should see a confirmation with text {string}', async function (expectedText) {
    await this.page.waitForSelector('.ticket__check-title');
    const confirmText = await this.page.$eval('.ticket__check-title', el => el.textContent.trim());
...
When('I choose a session with a taken seat', async function () {
    ...
    await this.page.waitForSelector('.buying-scheme__chair_disabled.buying-scheme__chair_taken');
});

// Занятое место
When('I try to click a taken seat', async function () {
    const seat = await this.page.$('.buying-scheme__chair_disabled.buying-scheme__chair_taken');
    this.takenSeat = seat;
...
Then('the {string} button should be disabled', async function (buttonText) {
    ...
    const isDisabled = await this.page.$eval('.acceptin-button', el => el.disabled);
